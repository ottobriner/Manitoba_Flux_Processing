...
input<-read.csv(paste('./processed/ATMOS_L1.csv',sep=''))
...
met <- data.frame()
# Get header names
names_temp <- names(read.csv(paste("/home/otto/data/atmos-flux-data/output/atmos484_20230131_fluxnet.csv")))

#Load data and apply header names
temp <- read.csv(paste("/home/otto/data/atmos-flux-data/output/atmos484_20230131_fluxnet.csv"))
...
# trimming to dates of flux file
met <- met[pmatch(Data_QAQC$DATE, met$DATE),]

start_rad <- min(which(!is.na(met$NR)))
end_rad <- max(which(!is.na(met$NR)))
met <- met[start_rad:end_rad,]

# fill net radiation forward (assuming only every other half hour is missing)
met <- met %>% 
  fill(NR)

# trim met and flux data to dates of met tower data (net radiation)
met <- met[!is.na(met$NR),]
...
output <- cbind(met[M_start:M_end,match(c('year','DOY_START','hour','TA_EP','RN'),names(met))], Data_QAQC[F_start:F_end,match(c('co2_flux','u.','wind_dir'),names(Data_QAQC))])
...
output <- output[c("year","DOY", "hour","co2_flux","RN","TA_EP","u.","wind_dir")]
...
#Saving the file
write.table(output, file = paste('/home/otto/data/atmos-flux-data/processed/REddyProc_input/for_ustar_filtering','.txt',sep=''),row.names=FALSE,sep='\t') 
```

```{r Ustar filtering in REddyProc, echo=FALSE, warning=FALSE, include=FALSE}
# Load data
EddyData.F <- fLoadTXTIntoDataframe("/home/otto/data/atmos-flux-data/processed/REddyProc_input/for_ustar_filtering.txt")
...
write.csv(Data_QAQC,paste('/home/otto/data/atmos-flux-data/processed/ATMOS_L2','.csv',sep=''),row.names=FALSE)   
```
...
output <- cbind(met[,c('year','DOY_local','hour','TA_EP','NR','RH_EP', 'VPD_hPa','Tsoil_mean')], 
...
output <- output[c("year", "DOY_local", "hour","co2_flux","ch4_flux","LE","H","NR","TA_EP","Tsoil_mean", "RH_EP","VPD_hPa","u.","wind_dir")]
...
#Saving biomet file:
write.csv(met,paste('/home/otto/data/atmos-flux-data/processed/Gapfilled_biomet','.csv',sep=''),row.names=FALSE)   
#Saving the file:
write.table(output, file = paste('/home/otto/data/atmos-flux-data/processed/REddyProc_input/for_gap_filling_partitioningATMOS','.txt',sep=''),row.names=FALSE,sep='\t')   
```
