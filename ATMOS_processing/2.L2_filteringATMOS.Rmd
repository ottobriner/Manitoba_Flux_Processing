...
met <- data.frame()
# Get header names
names_temp <- names(read.csv(paste("/home/otto/data/atmos-flux-data/output/atmos484_20230121114953.csv")))

#Load data and apply header names
temp <- read.csv(paste("/home/otto/data/atmos-flux-data/output/atmos484_20230121114953.csv"))
names(temp) <- names_temp

# Append to file
met <- smartbind(met, temp, fill="NA")

# adding "year" and "hour" columns
met$year <- year(met$datetime)
met$hour <- hour(met$datetime)
...
met$Timestamp <- as.POSIXct(met$datetime)
