...
# read fluxnet
input<-read.csv(paste('./processed/ATMOS_L1_fluxnet_2023-04-02.csv',sep=''))
...
for(i in 1:length(met$Timestamp)){
  if (met$time[i]=='0000'){
    if (met$Timestamp[i] %in% c('2022-01-31 0000',
                        ...
                        '2022-12-31 0000',
                        '2023-01-31 0000')){
      if (met$date[i] == '2022-12-31'){
        met$Year[i] <- met$Year[i+1]
        met$Month[i] <- met$Month[i+1]
        met$DOM[i] <- met$DOM[i+1] # breaks if met stops on 0000h
      }else
      met$Month[i] <- met$Month[i+1] 
      met$DOM[i] <- met$DOM[i+1] # breaks if met stops on 0000h
    }else
...
met$es <- 0.611*exp(17.502*fluxnet$TA_EP[pmatch(met$Timestamp, fluxnet$DATE)]/(fluxnet$TA_EP[pmatch(met$Timestamp, fluxnet$DATE)]+240.97))
met$VPD <- met$es*(1-(fluxnet$RH_EP[pmatch(met$Timestamp, fluxnet$DATE)]/100))
