...
# set wd
getwd()
dir <- "/Users/darianng/Google\ Drive/My\ Drive/Micromet\ Lab/Projects/2021-Hogg/Flux-tower"
knitr::opts_knit$set(root.dir = dir)
```

```{r Load non-filtered data, echo=FALSE, include=FALSE}
input<-read.csv(paste('./flux_data/Hogg_L1.csv',sep=''))
input$DATE<-as.POSIXct(input$DATE,format="%Y-%m-%d %H:%M:%S", tz = "UTC")

input <- tail(input,-4) # Fixing issue where first 4 entries were NA's. Skipping first 4 rows. edit: Dec 1 2021, D.Ng
...
h2o_flux_min <- -1.5 #[mmol+1s-1m-2]
h2o_flux_max <- 30 #[mmol+1s-1m-2]
h2o_var_max <- 1500
...
Ndays <- as.numeric((difftime(ts$Timestamp[nrow(ts)],ts$Timestamp[1], "Etc/GMT-7",
...
write.csv(Data_QAQC,paste('./flux_data/Hogg_L2','.csv',sep=''),row.names=FALSE)   
```
...
#Saving the file:
write.table(output, file = paste('./flux_data/REddyProc_input/for_gap_filling_partitioningHogg','.txt',sep=''),row.names=FALSE,sep='\t')   
```
